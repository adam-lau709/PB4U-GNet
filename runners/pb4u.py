...
class Runner(nn.Module):
    ...
    def valid_rollout(self, sequence, repeat=None, n_steps=-1, bare=False, record_time=False, solve_collision=False):
        """
        ...
        trajectory, obstacle_trajectory, metrics_dict = self._rollout(sequence, n_samples, repeat,
                                                                      progressbar=True, bare=bare, solve_collision=solve_collision)
    ...
    def _rollout(self, sequence, n_steps, repeat=None, progressbar=False, bare=False, solve_collision=False):
        trajectory = []
        obstacle_trajectory = []
        ...
        for i in pbar:
            ...
            with torch.no_grad():
                # repeat=None triggers resolution-aware K computation inside the model;
                # pass an explicit value only to override (e.g. for ablation studies).
                state = self.model(state, repeat=repeat, is_training=False)
            
            if solve_collision:
                position = state['cloth'].pred_pos.clone()
                obstacle_pos = state['obstacle'].pos
                normals = state['obstacle'].normals
                collision_cloth_vertices = state['cloth', 'world_edge', 'obstacle'].edge_index[0]
                collision_obstacle_vertices = state['cloth', 'world_edge', 'obstacle'].edge_index[1]
                cloth_collision_pos = position[collision_cloth_vertices]
                obstacle_collision_pos = obstacle_pos[collision_obstacle_vertices]
                collision_normals = normals[collision_obstacle_vertices]
                
                eps = 0.005
                dot_prod = torch.sum((cloth_collision_pos - obstacle_collision_pos) * collision_normals, dim=1) - eps
                dot_prod[dot_prod > 0] = 0

                collision_offset = dot_prod.unsqueeze(1) * collision_normals
                position[collision_cloth_vertices] -= collision_offset

                trajectory.append(position.detach().cpu().numpy())
    ...
    def forward(self, sample, roll_steps=1, optimizer=None, scheduler=None) -> dict:
        ...
        for i in range(roll_steps):
            ...
            sample_step = self.model(sample_step, repeat)
...
def run_epoch(training_module: Runner, aux_modules: dict, dataloader: DataLoader,
    ...
    for sample in prbar:
        ...
        roll_steps = training_module.mcfg.roll_init + (global_step // training_module.mcfg.increase_roll_every)
        ...
        ld_str = ', '.join([f"{key[:2]}: {value:.4f}" for key, value in ld_to_write.items()])
        prbar.set_postfix_str(ld_str)
        avg_stretch_loss += ld_to_write['stretching_energy_loss']

        if global_step % 2000 == 0:
            print("=======================")
            print("Current loss: " + ld_str)
            print("Average stretch: " + str(avg_stretch_loss / 2000))
            print("=======================")
            avg_stretch_loss = 0
