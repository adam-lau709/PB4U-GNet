...
class CollisionPreprocessor:
    ...
    def solve(self, sample):
        ...
        for i in range(B):
            ...
            new_pos = cloth_pos - pos_shift * 1.5
            new_prev_pos = cloth_prev_pos - prev_pos_shift * 1.5
    ...
    def solve_pred(self, sample):
        B = sample.num_graphs
        new_example_list = []
        for i in range(B):
            example = sample.get_example(i)
            cloth_pos = example['cloth'].pred_pos.unsqueeze(0)
            obstacle_pos = example['obstacle'].target_pos.unsqueeze(0)
            obstacle_faces = example['obstacle'].faces_batch.T.unsqueeze(0)
            cloth_prev_pos = example['cloth'].pos.unsqueeze(0)

            pos_shift = self.calc_direction(cloth_pos, obstacle_pos, obstacle_faces)
            prev_pos_shift = self.calc_direction(cloth_prev_pos, obstacle_pos, obstacle_faces)

            new_pos = cloth_pos - pos_shift * 2
            new_prev_pos = cloth_prev_pos - prev_pos_shift * 2

            example['cloth'].pred_pos = new_pos[0]
            example['cloth'].pos = new_prev_pos[0]
            new_example_list.append(example)
