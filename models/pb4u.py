...
class Config:
    ...
    message_passing_steps: int = 15     # total number of message passing steps at base (lowest) training resolution
    l_base: float = 0.015                # mean edge length at base training resolution (metres); D = message_passing_steps * l_base
    n_coarse_levels: int = 0
...
class Model(nn.Module):
    def __init__(self, learned_model, mcfg: Config):
        ...
        # Physical propagation distance D = K_base * L_base
        self.propagation_distance = mcfg.message_passing_steps * mcfg.l_base
    ...
    def create_mesh_edge_set(self, sample, is_training, edge_label, normalizer):
        ...
        relative_rest_pos_direction = relative_rest_pos / relative_rest_pos_norm

        relative_length = relative_pos_norm / relative_rest_pos_norm  # (e, 1)
        ...
        if edge_label == 'mesh_edge':
            n_verts = sample['cloth'].pos.shape[0]
            receivers = edges[:, 1]  # (e,)
            length_sum = torch.zeros(n_verts, 1, device=rest_pos.device)
            count = torch.zeros(n_verts, 1, device=rest_pos.device)
            length_sum.index_add_(0, receivers, relative_rest_pos_norm)
            count.index_add_(0, receivers, torch.ones_like(relative_rest_pos_norm))
            sample['cloth'].per_vertex_edge_length = length_sum / count.clamp(min=1)  # (n_verts, 1)
            sample['cloth'].average_edge_length = torch.mean(relative_rest_pos_norm)
    ...
    def add_node_features(self, sample):
        ...
        for k in ['cloth', 'obstacle']:
            ...
            node_features = torch.cat(
                [velocity, vertex_type_emb, normals, timestep, v_mass, bending_coeff, lame_mu,
                 lame_lambda], dim=-1)
    ...
    def get_position(self, sample, is_training):
        ...
        # update scaling
        K = 100
        l = sample['cloth'].average_edge_length
        # l = sample['cloth'].per_vertex_edge_length
        acceleration *= K * l  # (n_verts, 1) broadcasts over xyz
    ...
    def forward(self, sample, repeat=None, is_training=True):
        """
        Forward pass. Predicts accelerations for each garment node and computes their positions in the next frame.

        :param repeat: optional override for the number of propagation steps K. When None (default),
            K is computed automatically from the mesh's mean edge length using the resolution-aware
            propagation control formula K = floor(D / L̄) where D = message_passing_steps * l_base
            (paper Eq. 6–7). Pass an explicit integer only to override (e.g. for ablation studies).
        ...
        # compute the number of propagation steps
        if repeat is None:
            repeat = max(1, int(self.propagation_distance / sample['cloth'].average_edge_length.item()))
        sample = self._learned_model(sample, repeat)
