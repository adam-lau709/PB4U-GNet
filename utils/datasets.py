...
def load_garments_dict_with_level(path: str) -> Dict[str, dict]:
    """ Load the garments_dict containing data for all garments from a pickle file"""
    ...
    for garment, g_dict in garments_dict.items():
        for i in range(1, 4):
            g_dict[f'level{i}']['lbs'] = convert_lbs_dict(g_dict[f'level{i}']['lbs'])
...
def make_garment_smpl_dict_with_level(garments_dict: Dict[str, dict], smpl_model: SMPL) -> Dict[str, GarmentSMPL]:
    """ For each garment create a GarmentSMPL object"""
    garment_smpl_model_dict = dict()  # Initialize a regular dictionary
    for garment, g_dict in garments_dict.items():
        garment_smpl_model_dict[garment] = {}
        for i in range(1, 4):
            g_smpl_model = GarmentSMPL(smpl_model, g_dict[f'level{i}']['lbs'])
            garment_smpl_model_dict[garment][f'level{i}'] = g_smpl_model
