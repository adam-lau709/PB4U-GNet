...
class ClothWithResolution(Cloth):
    def __init__(self, material, always_overwrite_mass=False):
        super().__init__(material, True)
        self.always_overwrite_mass = always_overwrite_mass
    ...
    def set_batch(self, batch, overwrite_pos=False):
        ...
        for i in range(B):
            example = batch.get_example(i)
            garment_name = example.garment_name
            resolution = example['cloth'].resolution if 'resolution' in example['cloth'] else 0
            ...
            if f"{garment_name}_{resolution}" in self.cache:
                f_connectivity = self.cache[f"{garment_name}_{resolution}"]['f_connectivity']
                f_connectivity_edges = self.cache[f"{garment_name}_{resolution}"]['f_connectivity_edges']
                if not overwrite_pos:
                    if not self.always_overwrite_mass:
                        v_mass = self.cache[f"{garment_name}_{resolution}"]['v_mass']
                    f_area = self.cache[f"{garment_name}_{resolution}"]['f_area']
                    Dm_inv = self.cache[f"{garment_name}_{resolution}"]['Dm_inv']
                else:
                    if not self.always_overwrite_mass:
                        v_mass = self.make_v_mass(v, f, density, device)
                    f_area = self.make_f_area(v, f, device)
                ...
                self.cache[f"{garment_name}_{resolution}"]['v_mass'] = v_mass
                self.cache[f"{garment_name}_{resolution}"]['f_area'] = f_area
                self.cache[f"{garment_name}_{resolution}"]['f_connectivity'] = f_connectivity
                self.cache[f"{garment_name}_{resolution}"]['f_connectivity_edges'] = f_connectivity_edges
                self.cache[f"{garment_name}_{resolution}"]['Dm_inv'] = Dm_inv
